...
The release tagged above corresponds to the version of the code used to produce the figures and numerical results reported in this thesis. The repository contains the Simulink feeder model, the data-generation and training scripts, the self-healing/restoration routine, and the figure-generation scripts (all in `src/`); a lightweight CSV copy of the dataset (`outputs/dataset/fault_dataset_1000.csv`); the result summaries (`outputs/summaries/`); and the 30 figures used in the final thesis, grouped by chapter and named after their official captions (`outputs/figures/thesis_final_named/`).
...
| `gen_fig5_14.m` | Helper script that produces the post-restoration voltage figure (Fig. 5.13 in the final thesis; the script retains its original filename) from the restoration results table. |
