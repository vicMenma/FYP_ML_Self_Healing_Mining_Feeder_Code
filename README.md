...
tools/                                 Python generators for the document-only artefacts
  ...
  make_figure_c1_deployment_architecture.py   builds the Appendix C architecture figure
docs/                                  thesis PDF, bibliography, QA logs, and the project plan
  Project_Gantt_Chart.xlsx             work breakdown, dates, predecessors, milestones, % complete
```

The numbered figures used in the thesis are preserved in
...
The Appendix B project plan lives in [`docs/Project_Gantt_Chart.xlsx`](docs/Project_Gantt_Chart.xlsx)
— a formula-driven work breakdown with start and finish dates, durations, predecessors, milestones
and percentage completion, plus the Gantt chart drawn from those cells. In the thesis itself the
Gantt is reproduced as a Word table (Table B.1) rather than an image, so the spreadsheet is the
editable source of the dates rather than the figure source.
